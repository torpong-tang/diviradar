...
Flex Message ของ Daily Radar ควรแสดง:

- หุ้นเด่นพร้อมราคา, score, yield และสถานะ icon
- DCA Plan จากหุ้น score >= 80 สูงสุด 3 ตัว
- ราคา/หุ้น, จำนวนหุ้นตาม lot 100, งบที่จัดสรร และราคารวมที่จะซื้อ
- สถานะราคารวมอยู่ใน tolerance +/-5% หรือไม่
...
### XD sync integrity and schema rollout

Calendar update and per-stock history use the same sync service:

- `src/lib/market-data/settrade-xd-sync-service.ts` validates targets, dates, amounts and source rows
- identical source duplicates are collapsed before persistence
- conflicting duplicates for the same stock/XD date are rejected
- `src/lib/market-data/settrade-xd-repository.ts` persists accepted rows in a transaction
- SQLite enforces `@@unique([stockId, xdDate])`
- a partial sync updates the attempt timestamp but does not advance the successful sync timestamp

Before the first production rollout containing the compound unique key:

```bash
# 1. Back up the production database first.
sqlite3 /var/lib/2startup/diviradar/diviradar.db \
  ".backup '/var/backups/2startup/diviradar/diviradar-before-xd-unique.db'"

# 2. Inspect duplicates without changing data.
DATABASE_URL=file:/var/lib/2startup/diviradar/diviradar.db \
  npm run db:dedupe-dividends

# 3. Apply cleanup only when the dry run reports duplicates.
DATABASE_URL=file:/var/lib/2startup/diviradar/diviradar.db \
  npm run db:dedupe-dividends -- --apply

# 4. Apply the schema after backup and duplicate verification.
DATABASE_URL=file:/var/lib/2startup/diviradar/diviradar.db \
  npx prisma db push --accept-data-loss
```

The `--accept-data-loss` flag is required by Prisma for adding the unique index. Do not run it before the backup and duplicate check.

## DCA Plan

หน้า DCA Plan มี input งบ DCA รายเดือนและปุ่มคำนวณแผน โดย logic อยู่ที่ `src/lib/dca/dca-plan.ts`:

- ใช้เฉพาะหุ้น Radar ที่ score >= 80
- เลือกสูงสุด 3 หุ้น เรียงตาม score
- น้ำหนักเริ่มต้น 40% / 35% / 25%
- ปัดงบย่อยเป็นหลักร้อย
- ซื้อเป็น lot 100 หุ้น
- เลือกจำนวน lot ที่ทำให้ราคารวมใกล้งบที่จัดสรรที่สุด
- ระบุว่าอยู่ใน tolerance +/-5% หรือไม่

เมื่อ cron ส่ง LINE OA ระบบจะเรียก logic เดียวกันและแนบ DCA Plan ใน Flex Message ด้วย
...
- DCA Plan คำนวณหุ้น, ราคา/หุ้น, จำนวนหุ้น, งบจัดสรร และราคารวมได้
...
The permanent unit test suite currently covers Settrade XD target resolution, invalid rows, identical duplicates, conflicting duplicates, idempotent sync, partial source failure and out-of-month source data.
...
- DCA Plan calculation and LINE Flex payload fields
