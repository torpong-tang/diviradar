...
- DCA Plan แบบ rule-based เลือกหุ้น score >= 80 สูงสุด 3 ตัว พร้อมราคา/หุ้น จำนวนหุ้นแบบ lot 100 และราคารวม
- Settings สำหรับ DCA amount, Auto price update schedule และ LINE OA notification switcher
- LINE OA Flex Message สำหรับ Daily Radar พร้อมราคา, score, yield, สถานะหุ้น และ DCA Plan
- LINE OA webhook command `/id` สำหรับตอบกลับ `groupId`, `roomId` หรือ `userId` เพื่อช่วยตั้งค่า LINE Target ID
...
INITIAL_ADMIN_PASSWORD="<strong-password-minimum-12-characters>" \
npm run db:seed
npm run dev -- -p 3010
```
...
`db:seed` เป็น production-safe seed: เพิ่มเฉพาะค่าเริ่มต้นที่ยังไม่มีและจะไม่ลบข้อมูลหรือเปลี่ยนรหัสผ่านของผู้ใช้เดิม

หากต้องการรีเซ็ตฐานข้อมูล development เป็นข้อมูลตัวอย่าง ให้กำหนด `DEMO_ADMIN_EMAIL` และ `DEMO_ADMIN_PASSWORD` แล้วใช้ `npm run db:seed:demo` เท่านั้น คำสั่งนี้ลบ portfolio, alert, dividend, price และ notification log เดิม จึงห้ามใช้กับ production
...
npm run build
npm run db:push
npm run db:dedupe-dividends # dry-run duplicate check
npm run db:seed
npm run db:seed:demo # development only; destructive
```
...
- ใช้ unique key `(stockId, xdDate)` เพื่อป้องกันข้อมูล XD ซ้ำ
- source rows ที่ซ้ำและมีค่าเหมือนกันจะถูกยุบเป็นรายการเดียว
- source rows วันเดียวกันที่มีข้อมูลขัดแย้งจะไม่ถูกบันทึกและจะแจ้ง partial update
- `last_settrade_xd_sync_at` จะเปลี่ยนเฉพาะเมื่อทุกเดือน sync สำเร็จและไม่มีข้อมูลถูก reject
...
หน้า DCA Plan ใช้หลักการคำนวณแบบเรียบง่ายเพื่อช่วยวางแผนซื้อหุ้นปันผลรายเดือน:

- เลือกหุ้นจาก Radar ที่มี score ตั้งแต่ 80 ขึ้นไป
- เรียงตาม score สูงสุด และใช้ไม่เกิน 3 ตัวแรก
- แบ่งสัดส่วนโดยประมาณ 40% / 35% / 25%
- ปัดงบย่อยเป็นหลักร้อย
- คำนวณจำนวนหุ้นตาม lot ขั้นต่ำ 100 หุ้น
- เลือกจำนวนหุ้นที่ทำให้ราคารวมใกล้งบย่อยที่สุด
- แสดงสถานะว่าราคารวมอยู่ใน tolerance +/-5% หรือไม่

Daily Radar ที่ส่งผ่าน LINE OA จะแนบส่วน DCA Plan ด้วย โดยแสดงหุ้น, ราคา/หุ้น, จำนวนหุ้น, งบที่จัดสรร และราคารวมที่จะซื้อ
...
- ตั้งค่า `LINE_CHANNEL_SECRET` ใน server environment เพื่อใช้ตรวจ `x-line-signature` ของ webhook
...
- สำหรับ webhook `/id` ต้องมี `LINE_CHANNEL_SECRET` ถูกต้อง ไม่เช่นนั้นระบบจะปฏิเสธ request

Manual `Update Prices` ในหน้า app ใช้สำหรับอัปเดตราคาในระบบเท่านั้น และไม่ส่ง LINE OA

### LINE OA `/id` command

ถ้าเชิญ LINE OA เข้ากลุ่มแล้วพิมพ์ `/id` ในกลุ่ม ระบบ webhook จะตอบกลับด้วย `groupId` ของกลุ่มนั้น เพื่อให้นำไปกรอกใน Settings > LINE Target ID ได้ทันที

Webhook URL:

```text
https://2startup.cloud/diviradar/api/line/webhook
```

ผลลัพธ์:

- ใน group chat ตอบกลับ `DiviRadar LINE Group ID`
- ใน room chat ตอบกลับ `DiviRadar LINE Room ID`
- ใน private chat ตอบกลับ `DiviRadar LINE User ID`

ต้องเปิด `Use webhook` และ `Allow bot to join group chats` ใน LINE Developers Console พร้อมตั้ง `LINE_CHANNEL_SECRET` และ LINE Channel Access Token ฝั่ง server ระบบจะปฏิเสธ webhook ที่ลายเซ็นไม่ถูกต้อง และจะไม่ส่ง token เดิมกลับมายัง browser/API
...
- `LINE_CHANNEL_SECRET` ต้องมาจาก LINE Developers Console > Messaging API channel > Basic settings > Channel secret
...
- หากใช้ LINE OA ในกลุ่ม ให้พิมพ์ `/id` ในกลุ่มเพื่อตรวจ `groupId` แล้วนำค่าไปใส่ `LINE Target ID`
- ตั้ง `NOTIFICATION_LOG_RETENTION_DAYS` และ `NOTIFICATION_LOG_MAX_ROWS` เพื่อไม่ให้ `NotificationLog` โตต่อเนื่อง
